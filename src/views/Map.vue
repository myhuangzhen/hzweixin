    ...
    <div style="height:960px;margin:0px 30px 30px 30px" id="container" tabindex="0">
      <div id="map" class="mchart" style="margin-top: 10px;position: relative; height: 960px;">
        ...
        <div style="color: antiquewhite;width: 380px;height: 70px;line-height: 70px;text-align: left;position: absolute;font-size: 16px;float: left;font-weight:bold;margin-left:20px" id="left_name">
          <div style="float:left;width:50px;cursor:pointer;display:none" @click="goback" class="goback"><img src="@/assets/img/up.png" style="margin-bottom: 5px;" title="上一级"></div>
          <div style="float:left;font-size: 19px;">{{name}}</div>
        </div>
  ...
  methods: {
    goback: function () {
      if (this.name.indexOf('公司') !== -1) {
        ...
        axios.get('http://47.92.121.146:9002/service/map.selectmapshowtree?name=' + nodes[0].province).then(data => {
    ...
    freshArea: function () {
      axios.get('/service/map.selectallprovince').then(res => {
    ...
    zTreeOnClick: function (event, treeId, treeNode) {
      this.map.setZoom(9)
      if (this.name === '省级列表') {
        axios.get('/service/map.selectmapshowtree?name=' + treeNode.name).then(data => {
          $('.goback').show()
    ...
    initmarker: function (name) {
      ...
      this.map.clearMap()
      axios.get('/device/selectDeviceLocationInfo?subordinateUnit=' + name + '&keyword=&id=240A2D4E00FCCC470E013FDADD5BF919').then(data => {
        ...
        if (arrdata1.length === 0) {
          ...
          arrdata1.forEach(function (marker) {
            ...
            marker.on('click', function (e) {
              axios.get('service/video.GetCameraPlayURL?department=' + this.B.extData.subordinateUnit + '&cameraNum=' + this.B.extData.cameraNum + '&videoType=0&user=JTB000202001200001').then(stream => {
                console.log(stream.data.code)
